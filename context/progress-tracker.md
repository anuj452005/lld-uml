...
- **Auth: OAuth redirect base URL**
  ...
  - OAuth `redirect_to` uses `NEXT_PUBLIC_SITE_URL` first (`getOAuthRedirectBaseUrl`) so production Google sign-in returns to the Azure host, not localhost. Dockerfile does **not** set `NEXT_PUBLIC_SITE_URL` (Option A): CI omitted that build-arg and empty `ENV` overrode `frontend/.env.production` during `next build`; the committed `.env.production` value is used instead.
